...
const CRUDOperationsDemo = () => {
  ...
  const createTodo = async (e) => {
    ...
    try {
      ...
      // Karena JSONPlaceholder tidak benar-benar menyimpan, kita simulasi ID unik
      newTodo.id = Date.now(); 
    ...
    <div className="crud-demo">
      ...
      <div className="todos-section">
        ...
        {loading && !editingTodo ? (
          ...
          <div className="error-state">
            <p>{error}</p>
            <button onClick={fetchTodos} className="btn btn-secondary">Coba Lagi</button>
          </div>
        ) : (
            ...
            {todos.map(todo => (
              <div key={todo.id} className={`todo-card ${todo.completed ? 'completed' : ''}`}>
                <div className="todo-content">
                  ...
                  <p className="text-muted">ID: {todo.id}</p>
                </div>
                <div className="todo-actions">
                  <button onClick={() => startEditing(todo)} className="btn btn-warning btn-sm">
                    Edit
                  </button>
                  <button onClick={() => deleteTodo(todo.id)} disabled={loading} className="btn btn-danger btn-sm">
                    Delete
                  </button>
                </div>
              </div>
      ...
      <div className="api-info">
        ...
        <p>
          ...
          <strong> Data tidak benar-benar disimpan</strong> di server, tetapi kita bisa simulasi semua operasi CRUD.
        </p>
      </div>
    </div>
  );
